...
QJsonObject KeycardBridge::unpairCard()
{
    ...
    try {
        ...
        // Unpair on card (clears the pairing slot)
        ...
        if (!removed) {
            qWarning() << "KeycardBridge: Failed to remove pairing from storage (but card unpaired)";
        }

        qDebug() << "KeycardBridge: Unpaired successfully";
